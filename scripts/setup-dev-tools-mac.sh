...
# Git behavior that does not reformat machine-readable output.
...
# -- agent-check: verify the deterministic agent shell contract --
write_managed_script "$HOME/Scripts/bin/agent-check" <<'SCRIPT'
#!/usr/bin/env bash
# Verify the shell contract used by coding agents.
set -uo pipefail
...
for shell in zsh sh; do
    if command -v "$shell" >/dev/null 2>&1; then
        for command in git omp node npm python; do
            path="$("$shell" -c "command -v -- $command" 2>/dev/null || true)"
            [[ -n "$path" ]] && pass "$shell resolves $command: $path" || fail "$shell cannot resolve $command"
        done
    else
        fail "$shell is unavailable"
    fi
done

if command -v zsh >/dev/null 2>&1 && [[ -r "$HOME/.zshrc" ]]; then
    agent_env="$(env -i HOME="$HOME" PATH="$PATH" AI_AGENT=1 zsh -dfc '
        source "$1"
        printf "%s\n" "$GIT_PAGER" "$PAGER" "$GIT_TERMINAL_PROMPT" "$NO_COLOR"
    ' zsh "$HOME/.zshrc" 2>/dev/null)"
    [[ "$agent_env" == $'cat\ncat\n0\n1' ]] &&
        pass "agent output environment is deterministic" ||
        fail "agent output environment is not deterministic"
else
    fail "generated zsh configuration is unavailable"
fi

if [[ -r "$HOME/.omp/agent/config.yml" ]] && command -v omp >/dev/null 2>&1; then
    omp config get theme.dark >/dev/null 2>&1 &&
        pass "omp reads its global configuration" ||
        fail "omp does not read its global configuration"
else
    fail "omp configuration or binary is unavailable"
fi
...
# Remove the retired project scaffold only when the generator still owns it.
remove_managed_script "$HOME/Scripts/bin/new-project" \
    "project scaffolding is no longer provided"
...
success "Helper scripts written (clean-downloads, clone-work, clone-personal, backup-dotfiles, project-stats, health-check, agent-check, setup-ssh, export-brewfile — merged, edits outside the markers are kept)"
...
if [[ "$DRY_RUN" == "true" ]]; then
    ...
    # Keep this global layer limited to durable preferences. Repository-local files
    # own workflow and documentation policy.
    emit_agent_preferences "Oh My Pi" | write_generated "$OMP_DIR/AGENTS.md"
    success "omp: global preferences written ($OMP_DIR/AGENTS.md)"

    # Kiro uses the same small global preference layer.
    emit_agent_preferences "Oh My Pi" | write_generated "$KIRO_AGENTS"
    success "Kiro global preferences written ($KIRO_AGENTS)"
...
# Agent shells require plain output and must fail instead of opening credential prompts.
if [[ -n "${AI_AGENT:-}" ]]; then
...
# Prompt, history, completion, and TUI initialization are human-terminal features.
if [[ -o interactive && -z "$AI_AGENT" ]]; then
